...
if command -v dpkg-deb >/dev/null 2>&1 && test -f "$dist/purrfind-0.5.0-rc2-x86_64.deb"; then
...
if command -v rpm >/dev/null 2>&1 && test -f "$dist/purrfind-0.5.0-rc2-x86_64.rpm"; then
    ...
    check 'RPM metadata and OCR requirements' sh -c \
        ...
        "$dist/purrfind-0.5.0-rc2-x86_64.rpm" "$rpm_db"
else
    printf 'SKIP RPM inspection (rpm or artifact unavailable)\n'
fi

if command -v pacman >/dev/null 2>&1 && test -f "$dist/purrfind-0.5.0rc2-26-x86_64.pkg.tar.zst"; then
    check 'Arch metadata and OCR requirements' sh -c \
        ...
        "$dist/purrfind-0.5.0rc2-26-x86_64.pkg.tar.zst"
else
